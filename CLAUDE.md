...
TypeScript CLI + library (`sessionlens`, npm package `sessionlens`) that analyzes AI agent sessions. Claude Code is the default `SessionAdapter`, not the only source; normalized JSON/JSONL events are supported via the `file` adapter.
...
- A recommendation's `risk` (`safe`/`review`/`verify`) gates `apply`; `apply` never edits provider config or a running session, only `~/.sessionlens/decisions.json`.
...
The very first release of a new package name can't use Trusted Publishing — npm requires the package to already exist before a trusted publisher can be attached. Run `npm publish --access public` by hand once, then configure Trusted Publishing on npmjs.com (package Settings → Trusted Publisher → GitHub Actions → owner `Nasrallah-AL`, repo `sessionlens`, workflow `release.yml`) for every release after that.
