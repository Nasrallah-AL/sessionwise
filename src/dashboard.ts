...
function recommendationCard(recommendation: Recommendation): string {
  ...
  return `<article class="recommendation searchable shade-${category}" data-recommendation-category="${category}" data-search="${escapeHtml(searchable)}">
    ...
    <footer><code>${escapeHtml(recommendation.kind)}</code>${recommendation.sessionId ? `<a href="#session-${escapeHtml(recommendation.sessionId)}" data-session-link="${escapeHtml(recommendation.sessionId)}">Open session</a>` : ""}</footer>
  </article>`;
}
...
export function generateDashboard(analysis: Analysis, relevance?: RelevanceReport): string {
  ...
  const relevanceRows = relevanceGroups.flatMap((group) => RELEVANCE_KINDS.flatMap((kind) => {
    ...
    return [`<tr class="searchable" data-search="${escapeHtml(searchable)}">
      <td><a href="#session-${escapeHtml(group.sessionId)}" data-session-link="${escapeHtml(group.sessionId)}">${escapeHtml(group.sessionId)}</a></td>
...
function switchView(view){
  state.view=view;
  document.querySelectorAll("[data-view]").forEach(item=>item.setAttribute("aria-selected",String(item.dataset.view===view)));
  applyFilters();
}
document.querySelectorAll("[data-session-link]").forEach(link=>link.addEventListener("click",event=>{
  event.preventDefault();
  switchView("sessions");
  const row=document.getElementById("session-"+link.dataset.sessionLink);
  if(!row)return;
